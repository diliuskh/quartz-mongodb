...
public class JobConverter {
  ...
  /**
   * Converts job detail into document. Depending on the config, job data map can be stored as a
   * {@code base64} encoded (default) or plain object.
   */
  ...
  public JobDetail toJobDetail(Document doc) throws JobPersistenceException {
    try {
      ...
      Class<Job> jobClass =
          (Class<Job>) loadHelper.getClassLoader().loadClass(doc.getString(JOB_CLASS));
  ...
  /**
   * Converts document into job data map. Will first try {@link JobDataConverter} to deserialize
   * from '{@value Constants#JOB_DATA}' ({@code base64}) or '{@value Constants#JOB_DATA_PLAIN}'
   * fields. If didn't succeed, will try to build job data from root fields (legacy, subject to
   * remove).
   */

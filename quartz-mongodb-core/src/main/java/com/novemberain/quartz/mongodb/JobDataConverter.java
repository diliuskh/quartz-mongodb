...
public class JobDataConverter {
  ...
  /**
   * Converts from job data map to document. Depending on config, job data map can be stored as a
   * {@code base64} encoded or plain object.
   *
  ...
  /**
   * Converts from document to job data map. If {@code base64} is preferred, tries to decode from
   * '{@value Constants#JOB_DATA}' field. Otherwise, first reads a plain object from '{@value
   * Constants#JOB_DATA_PLAIN}' field, or, if not present, falls back to {@code base64} field.
   *
  ...
  /**
   * Converts from document to job data map reading {@code base64} encoded field '{@value
   * Constants#JOB_DATA}'.
   */
  ...
  /**
   * Converts from document to job data map reading a plain object from field '{@value

...
public class SchedulerDao {
  ...
  public SchedulerDao(
      MongoCollection<Document> schedulerCollection,
  ...
  public Scheduler findInstance(String instanceId) {
    ...
    if (doc != null) {
      ...
      log.debug(
          "Returning scheduler instance '{}' with last checkin time: {}",
          scheduler.getInstanceId(),
          scheduler.getLastCheckinTime());
    } else {
   ...
   *
   * <p>The scheduler is selected based on its name, instanceId, and lastCheckinTime. If the last
   * check-in time is different, then it is not removed, for it might have gotten back to live.
   *
  ...
  private Bson createSchedulerFilter(
      String schedulerName, String instanceId, long lastCheckinTime) {
    return Filters.and(
  ...
  private Bson createSchedulerFilter(String schedulerName, String instanceId) {
    return Filters.and(
        Filters.eq(SCHEDULER_NAME_FIELD, schedulerName), Filters.eq(INSTANCE_ID_FIELD, instanceId));
  }

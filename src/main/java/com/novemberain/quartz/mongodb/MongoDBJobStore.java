...
public class MongoDBJobStore implements JobStore, Constants {
  ...
  /**
   * Set the frequency (in milliseconds) at which this instance "checks-in" with the other instances
   * of the cluster.
   *
   * <p>Affects the rate of detecting failed instances.
   */
  ...
  /**
   * Configures the way job data is stored. {@link JobDetail}'s or {@link Trigger}'s {@link
   * JobDataMap} can be represented as a {@code Map<String,Object>}.
   *
   * <ul>
   *   <li><b>{@code true}</b> (default) - Serialize map with {@link java.io.ObjectOutputStream
   *       ObjectOutputStream} and store as {@code base64} encoded string in field '{@value
   *       Constants#JOB_DATA}'. Map may contain any {@link java.io.Serializable Serializable}
   *       object internally, but will have some performance impact.
   *   <li><b>{@code false}</b> - Store map directly in '{@value Constants#JOB_DATA_PLAIN}' field.
   *       Use this option is you only store simple types in job data map for better performance.
   * </ul>
   */
  ...
  public void triggeredJobComplete(
      OperableTrigger trigger, JobDetail job, CompletedExecutionInstruction triggerInstCode) {

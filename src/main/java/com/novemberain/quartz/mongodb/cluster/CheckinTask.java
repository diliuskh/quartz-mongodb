...
public class CheckinTask implements Runnable {
   ...
   *
   * <p>If a scheduler cannot register itself due to an exception we stop JVM to prevent concurrent
   * execution of the same jobs together with other nodes that might have found this scheduler as
   * defunct and take over its triggers.
   */
  private static final Runnable DEFAULT_ERROR_HANDLER =
      () -> {

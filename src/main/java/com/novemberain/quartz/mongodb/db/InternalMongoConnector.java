...
public class InternalMongoConnector implements MongoConnector {
   ...
   */
  private InternalMongoConnector(
      final WriteConcern writeConcern, final MongoClient mongoClient, final String dbName) {
   ...
   */
  public InternalMongoConnector(
      final WriteConcern writeConcern, final String uri, final String dbName)
  ...
  private static MongoClient createClient(final MongoClientURI uri)
      throws SchedulerConfigException {
    try {
  ...
  /**
   * Creates an instance of MongoClient from server addresses, credentials and options wrapping
   * exception.
   */
